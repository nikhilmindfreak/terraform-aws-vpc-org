...
resource "aws_subnet" "public" { # first name is public[0], second name is public[1]
  count = length(var.public_subnet_cidrs)  # we use count beacuse we have 2 publicsubnets 
...
resource "aws_db_subnet_group" "default" {     # we group all subnets of data base
  name       = "${local.resource_name}"
  subnet_ids = aws_subnet.database[*].id   # subnet group is list and , * all
...
resource "aws_route" "public_route" {
  ...
  destination_cidr_block    = "0.0.0.0/0"     #internet gateway
  gateway_id = aws_internet_gateway.gw.id
}

resource "aws_route" "private_route_nat" {  # for private we give nat for outboud traffic
...
resource "aws_route_table_association" "public" {
  count = length(var.public_subnet_cidrs)   # we use count beacuse we have 2 publicsubnets 
  subnet_id      = element(aws_subnet.public[*].id, count.index)  # we have 2 elements , we use element
  route_table_id = aws_route_table.public.id
}
